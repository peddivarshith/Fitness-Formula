...
wrist_r = None;wrist_l = None;shoulder_r = None;shoulder_l = None;
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            try:
                ...
                # Calculate angle
                angle1 = calculate_angle(shoulder_r, elbow_r, wrist_r)
                angle2 = calculate_angle(shoulder_r, hip_r, knee_r)

                # Curl counter logic
                if angle2 <= 168:
                    if angle1 <= 100 and stage == "Forward":
                        stage = "Backward"
                        st.session_state.counter += 0.5
                    elif angle1 >= 160 and stage == "Backward":
                        stage = "Forward"
                        st.session_state.counter += 0.5
